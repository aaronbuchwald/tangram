...
//! model, with typed access via autosurgeon, optional disk persistence, and a
//! change signal that the embedding host turns into SSE streams and sync
//! wake-ups. This module is platform-portable: the signal is a plain
//! callback (the native host wires it to a watch channel; the WASM guest
//! runs without one).
...
impl<M: Model + Actions> Ctx<M> {
    /// Wrap a store handle. Host adapters call this; app code receives a
    /// `Ctx` and never constructs one.
    pub fn new(store: Arc<Store<M>>) -> Self {
        Self { store }
    }
...
pub struct Store<M> {
    ...
    /// Change signal: fired after every document change (local commit or
    /// sync receive). The native host points this at a watch channel that
    /// drives SSE streams and sync peers; in-memory guest stores leave it
    /// unset.
...
impl<M: Model + Actions> Store<M> {
    ...
    /// `on_change` fires after every change (see [`Store::bump`]).
    pub fn open(
        path: PathBuf,
        ...
        Ok(Self::from_doc(doc, Some(path), Some(Box::new(on_change))))
    }
    ...
    pub fn in_memory(bytes: &[u8]) -> anyhow::Result<Self> {
        Ok(Self::from_doc(AutoCommit::load(bytes)?, None, None))
    }
    ...
    /// Fire the change signal (called after local actions and after changes
