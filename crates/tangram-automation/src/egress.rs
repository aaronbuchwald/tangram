...
//! canonicalize once, match on parsed components. The host/path canonical form
//! comes from THE shared seam — the [`tangram_egress`] leaf crate, the same
//! canonicalizer the host's `http_fetch` enforcer and the manifest verifier's
//! call-grain arm use — so the browser fence and the component fence can never
//! disagree on what a host/URL means. This module adds only what is specific to
//! a browser request the gate sees as a RAW URL string: splitting the authority
//! (userinfo/port/IPv6) out of the URL, and a conservative DNS-grammar
//! admission check layered on top of the shared canonical host (defense in
//! depth — a host the shared seam canonicalizes but that is not a clean DNS
//! name or IP literal is still refused at the browser boundary).
    ...
    pub fn decide(&self, method: &str, url: &str) -> Decision {
        ...
        for rule in &self.path_denies {
            if rule.host == host
                ...
                && path_under_prefix(&path, &rule.path_prefix)
            {
...
/// Whether canonical `path` falls under canonical `prefix` at a SEGMENT
/// boundary — `prefix` itself, or any path below it. Both are already
/// canonicalized (trailing slash stripped by the shared seam), so `/gp/buy`
/// covers `/gp/buy` and `/gp/buy/now` but NOT the sibling `/gp/buyer`. A raw
/// `starts_with` would mis-deny the sibling; the boundary check is the whole
/// point of canonicalizing on segments rather than strings.
...
/// Canonicalize a host through THE shared egress seam
/// ([`tangram_egress::canonical_host`] — lowercase, strip trailing dot, reject
/// null-byte/empty), then apply a conservative DNS-grammar admission check on
/// top (defense in depth at the browser boundary). Returns `None` — fail closed
/// — for a host the shared seam refuses OR that is not a clean DNS name / IP
/// literal. (IDNA/punycode normalization is future work; ASCII hosts are
/// handled here and non-ASCII fails closed, the safe direction.)
pub fn canonicalize_host(host: &str) -> Option<String> {
    // The shared canonical form first — one canonicalizer for the whole fleet.
    let lowered = tangram_egress::canonical_host(host).ok()?;
    // Conservative allowed byte set: DNS labels, IPv4 dots, and the IPv6
    // hex/colon set. Reject everything else (this rejects '@', '/', whitespace,
    // '%', non-ASCII — all bypass levers the browser route hook must refuse).
    let ok = lowered
        .bytes()
    ...
    // No empty labels (a `..` or leading dot is a malformed host used to
    // confuse suffix matchers).
...
/// Canonicalize a path through THE shared egress seam
/// ([`tangram_egress::canonical_path`] — percent-decode, normalize `.`/`..`
/// dot-segments, collapse to an absolute path, strip a trailing slash). Sharing
/// the host's canonicalizer is what stops `/%2e%2e/` and `/a/../` style bypasses
/// of a call-level deny from being interpreted differently here than at the
/// component fence.
pub fn canonicalize_path(path: &str) -> String {
    tangram_egress::canonical_path(path)
}
