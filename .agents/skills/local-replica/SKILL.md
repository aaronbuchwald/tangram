...
argument-hint: connect|status|stop [--remote <ws base>] [--bind <addr:port>] [--data-dir <dir>]
allowed-tools: Bash
---
...
After running, relay the printed URLs to the user. If connect fails, read the
log path it prints (common causes: tunnel not up, bind port already in use).
