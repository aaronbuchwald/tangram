...
if [ -n "$WASM" ]; then
  # ── wasm mode: run the repo fleet locally, FEDERATED with the remote ──────
  # A dev replica has the repo and builds its components locally, so we run
  # the SAME base fleet the repo's apps.toml defines (each app's data synced
  # via a per-app `remote`) AND federate the registry document with the
  # remote (RUNTIME_PLAN Phase 9) so apps INSTALLED at runtime on any host
  # propagate here too. (The remote's base fleet is file-bootstrapped, which
  # does not live in the registry document — so registry-only bootstrap would
  # bring up nothing but the registry itself; we mirror the repo fleet
  # directly instead. A zero-build replica of a remote whose fleet is
  # published as url+hash artifacts would instead arrive purely via the
  # federated registry — that path is unchanged.)
  WASM_APPS=()
  while IFS= read -r app; do WASM_APPS+=("$app"); done < <(
    python3 - "$DIR/apps.toml" <<'PY'
import sys, tomllib
doc = tomllib.load(open(sys.argv[1], "rb"))
for name in doc.get("apps", {}):
    print(name)
PY
  )
  ...
  for app in "${WASM_APPS[@]}"; do mkdir -p "$DATA_DIR/$app"; done
  ...
  # (emitted per-app by the transform below when --remote-token is set).
  # Transform the repo apps.toml into the replica's: each app keeps its
  # registry/require_auth/allow_hosts/env, but its component points at the
  # LOCAL wasm build, its data_dir is per-app under the replica dir, and it
  # gains `remote = <remote>/<app>/sync` so its document syncs (the registry's
  # remote is what federates fleet membership). Secrets stay per-host — only
  # ${VAR}-style env references are copied, never resolved values.
  REMOTE_TOKEN_TOML=""
  [ -n "$REMOTE_TOKEN" ] && REMOTE_TOKEN_TOML='remote_token = "${TANGRAM_REMOTE_TOKEN}"'
  python3 - "$DIR/apps.toml" "$DIR" "$ABS_DATA" "$REMOTE" "$REMOTE_TOKEN_TOML" > "$APPS_TOML" <<'PY'
import sys, tomllib
repo_toml, dir_, data, remote, token_line = sys.argv[1:6]
doc = tomllib.load(open(repo_toml, "rb"))
def s(v): return '"' + str(v).replace('\\', '\\\\').replace('"', '\\"') + '"'
print("# Generated by replica.sh connect --wasm — the repo fleet, run locally")
print(f"# and federated with {remote} (each app's data + the registry document).")
for name, spec in doc.get("apps", {}).items():
    print(f"\n[apps.{name}]")
    print(f"component = {s(f'{dir_}/target/wasm32-wasip2/release/{name}.wasm')}")
    print(f"ui = {s(f'{dir_}/apps/{name}/ui')}")
    print(f"data_dir = {s(f'{data}/{name}')}")
    print(f"remote = {s(f'{remote}/{name}/sync')}")
    if token_line:
        print(token_line)
    if spec.get("registry"):
        print("registry = true")
    if spec.get("require_auth"):
        print("require_auth = true")
    if spec.get("allow_hosts"):
        print("allow_hosts = [" + ", ".join(s(h) for h in spec["allow_hosts"]) + "]")
    env = spec.get("env", {})
    if env:
        print(f"\n[apps.{name}.env]")
